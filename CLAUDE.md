...
- The personal identity has no `core.sshCommand` override and no `ssh/config` `Host` entry — it relies on ssh's own default key lookup (`~/.ssh/id_ed25519`, tried automatically for any host), so a personal-only machine needs zero SSH config beyond generating that key. `ssh/config` only defines aliased `Host` entries for the two work identities (`victoria-github`, `pc-bitbucket`), plus a URL rewrite in `.gitconfig` (`url "git@github.com-victoria-id:victoria-id/" insteadOf ...`) so victoria-id remotes transparently use the aliased SSH host.
